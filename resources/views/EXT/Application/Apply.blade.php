...
<script>
    ...
    <div class="card-body">
        <div class="row">
            <div class="col-md-6">
                <p><strong>Designation:</strong> {{ $extjobs->Designation }}</p>
                <p><strong>Job Group:</strong> {{ $extjobs->Job_Group }}</p>
                <p><strong>Proposed Number of Positions:</strong> {{ $extjobs->Proposed_No_of_Positions }}</p>
                <p><strong>Level  of  Education:</strong> {{ $extjobs->level }}</p>
               
            </div>
            <div class="col-md-6">
               
                <p><strong>Reference Number:</strong> {{ $extjobs->Ref_NO }}</p>
                <p><strong>Date From:</strong> {{ $extjobs->datefrom }}</p>
                <p><strong>Deadline:</strong> {{ $extjobs->deadline }}</p>
                <p><strong>Status:</strong> {{ $extjobs->status }}</p>
            </div>
        </div>
    </div>
</div>
...
<form class="row g-3" action="{{ route('EXT.Apply.Job', ['id' => $extjobs->id]) }}" method="POST"  enctype="multipart/form-data">
    @csrf
