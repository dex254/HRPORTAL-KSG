...
<div class="progress-tracker">
    <div class="step" data-step="1" onclick="navigateToRoute('{{ route('HRPU.Dashboard') }}')">
        <div class="hexagon">1</div>
        <span class="step-label">Bio data</span>
    </div>
    <div class="step" data-step="2" onclick="navigateToRoute('{{ route('Academic.Ext') }}')">
        <div class="hexagon">2</div>
        <span class="step-label">Academic</span>
    </div>
    <div class="step" data-step="3" onclick="navigateToRoute('{{ route('Special.ProfessionalBodyext') }}')">
        <div class="hexagon">3</div>
        <span class="step-label">Professional Body</span>
    </div>
    
    
    <div class="step" data-step="4" onclick="navigateToRoute('{{ route('Experience.Ext') }}')">
        <div class="hexagon">4</div>
        <span class="step-label">Experience</span>
    </div>
    <div class="step" data-step="5" onclick="navigateToRoute('{{ route('Experience.Teaching') }}')">
        <div class="hexagon">5</div>
        <span class="step-label">Teaching <br>Experience</span>
    </div>
    <div class="step" data-step="6" onclick="navigateToRoute('{{ route('Experience.Other') }}')">
    ...
    <div class="step" data-step="7" onclick="navigateToRoute('{{ route('Report.Ext') }}')">
        <div class="hexagon">7</div>
        <span class="step-label">Report</span>
    </div>
    
    <div class="step" data-step="8" onclick="navigateToRoute('{{ route('JOB.applicantsext') }}')">
        <div class="hexagon">8</div>
        <span class="step-label">Jobs</span>
    </div>
    <div class="step" data-step="9" onclick="navigateToRoute('{{ route('HRPU.Myapplicants') }}')">
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const stageMapping = {
            ...
            "Report.Ext": 7,
            "JOB.applicantsext": 8,
            "HRPU.Myapplicants": 9, // Mark all steps as complete for this route
            "JOB.Applyext": 10// Mark all steps as complete for this route as well
        };
        ...
        if (currentRoute === "HRPU.Myapplicants" || currentRoute === "JOB.Applyext"|| currentRoute ==="JOB.Applicationdetailsext") {
            document.querySelectorAll(".step").forEach(step => {
                step.classList.add("active");
            });
