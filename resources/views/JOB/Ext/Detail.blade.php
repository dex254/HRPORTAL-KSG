...
<!--end wrapper--><div class="modal fade" id="updateJobModal" tabindex="-1" aria-labelledby="updateJobModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('JOB.updateext', ['id' => $jobexts->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                        <label>Area</label>
                        <input type="text" class="form-control" name="area" value="{{ $jobexts->area }}" required />
                    </div>

                         <div class="col-md-6">
                        <label>Specialization</label>
                        <input type="text" class="form-control" name="Specialization" value="{{ $jobexts->Specialization }}" required />
                    </div>
    ...
    <select class="custom-select" name="level" id="levelOfEducation" required>
        ...
        <option value="O level" {{ $jobexts->level == 'O level' ? 'selected' : '' }}>O level</option>
        <option value="A level" {{ $jobexts->level == 'A level' ? 'selected' : '' }}>A level</option>
        <option value="Certificate" {{ $jobexts->level == 'Certificate' ? 'selected' : '' }}>Certificate</option>
        <option value="Diploma" {{ $jobexts->level == 'Diploma' ? 'selected' : '' }}>Diploma</option>
        <option value="Bachelor's Degree" {{ $jobexts->level == "Bachelor's Degree" ? 'selected' : '' }}>Bachelor's Degree</option>
        <option value="Master's Degree" {{ $jobexts->level == "Master's Degree" ? 'selected' : '' }}>Master's Degree</option>
        <option value="Doctorate" {{ $jobexts->level == 'Doctorate' ? 'selected' : '' }}>Doctorate (PhD)</option>
        <option value="Other" {{ $jobexts->level == 'Other' ? 'selected' : '' }}>Other</option>
    </select>
</div>


                    <div class="col-md-6">
                        <label>Proposed No of Positions</label>
                        <input type="number" class="form-control" name="Proposed_No_of_Positions" value="{{ $jobexts->Proposed_No_of_Positions }}" />
                    </div>
